...
def terminate_instance(name):
    return ec2c.terminate_instances(name)
...
def get_vpc_info(vpc, availability_zone=None):
    try:
        ...
        # subnet = list(ec2.subnets.filter(Filters=subnet_filters))
...
class LaunchSpecs:
    def __init__(self, vpc, instance_type='t2.micro', volume_size=20, delete_ebs=True, ami=None, availability_zone=None):
        ...
        self.keypair_name = f'aws-key-spot-instance'
...
def connect_to_instance(instance, keypath=f'{Path.home()}/.ssh/aws-key-spot-instance.pem', username='ubuntu', timeout=10):
...
# create_spot_instance(instance_name, launch_specs);